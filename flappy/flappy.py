...
def create_pipe():
    ...
    top_pipe = pipe_surface.get_rect(midbottom = (288, random_pipe_pos - 150))
...
game_font = pygame.font.Font('Elegant.ttf', 30)
...
#bird_surface = pygame.image.load('image/bluebird-midflap.png').convert_alpha()
#bird_rect = bird_surface.get_rect(center = (50, 256))
bird_midflap = pygame.image.load('image/bluebird-midflap.png').convert_alpha()
bird_downflap = pygame.image.load('image/bluebird-downflap.png').convert_alpha()
bird_upflap = pygame.image.load('image/bluebird-upflap.png').convert_alpha()
...
death_sound = pygame.mixer.Sound('sound/point.wav')
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if pygame.mouse.get_pressed()[0] and game_active:
                bird_movement = 0
                bird_movement -= 6
                flap_sound.play()
            if pygame.mouse.get_pressed()[0] and game_active == False:
                game_active = True
                pipe_list.clear()
